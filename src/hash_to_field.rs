...
use fff::Field;
use hkdf::Hkdf;
use sha2ni::digest::generic_array::{ArrayLength, GenericArray};
...
impl<T: FromRO> HashToField<T> {
    ...
    pub fn new<B: AsRef<[u8]>>(msg: B, dst: Option<&[u8]>) -> HashToField<T> {
        let mut msg_padded = msg.as_ref().to_vec();
        msg_padded.push(0u8);
...
pub trait FromRO {
    /// Gives the result of hash_to_field(msg, ctr) when input = H(msg).
